...
function showPanel(id, el, event) {
  ...
  if (typeof showSitePage === 'function') showSitePage('app', false);
  ...
  // Website-style tab behavior: change the app content without yanking the user
  // back to the page top. If the app is not visible yet, move to the workflow.
  const workflow = document.getElementById('workflow');
  const workflowTop = workflow ? workflow.getBoundingClientRect().top + window.scrollY - 84 : 0;
  const currentlyAboveApp = workflow && window.scrollY < workflowTop - 120;
  if (currentlyAboveApp) {
    requestAnimationFrame(() => window.scrollTo({ top: workflowTop, left: 0, behavior: 'smooth' }));
  }
...
function currentWeights(){
  ...
  return {
    gla: val('w-gla'),
    distance: val('w-distance'),
    date: val('w-date'),
    site: val('w-site'),
    year: val('w-year'),
    garage: val('w-garage'),
    basement: val('w-basement'),
    pool: val('w-pool'),
    qual: val('w-qual'),
    cond: val('w-cond'),
    price: val('w-price'),
...
document.addEventListener('DOMContentLoaded', function(){
  window.safeShowPanel = function(id, el, event){
    return showPanel(id, el, event);
  };
...
// Vite module compatibility: inline HTML handlers and generated buttons must be global.
try { window.handleCSV = handleCSV; } catch (e) {}
try { window.parseCSVFile = parseCSVFile; } catch (e) {}
try { window.updateMTSmoothingHelp = updateMTSmoothingHelp; } catch (e) {}
try { window.importWorkfileJSON = importWorkfileJSON; } catch (e) {}
try { window.showPanel = showPanel; } catch (e) {}
try { window.showSub = showSub; } catch (e) {}
...
/* ===== WEBSITE FLOW FIXES: LANDING / AUTH / APP SHELL ===== */
(function(){
  ...
  function ensureAuthPage(){
    ...
    auth.innerHTML = `
      <div class="auth-card">
        <div class="brand-lockup"><span class="brand-mark" aria-hidden="true"></span><span class="brand-name">ValoraIQ<small>Secure project workspace</small></span></div>
        <div class="auth-tabs" role="tablist" aria-label="Login or sign up">
        ...
        <div class="auth-panel active" id="auth-login">
          <h2>Welcome back</h2>
          <p>Open your valuation workspace. This prototype uses local browser storage; connect a real auth provider before collecting live client data.</p>
          <label>Email</label><input type="email" placeholder="you@example.com" autocomplete="email">
          <label>Password</label><input type="password" placeholder="••••••••" autocomplete="current-password">
          <button type="button" class="btn btn-gold auth-continue">Continue to Workspace</button>
        </div>
        <div class="auth-panel" id="auth-signup">
          <h2>Create your workspace</h2>
          <p>Start a ValoraIQ project workspace for appraiser workfile support or agent CMA presentations.</p>
          <label>Name</label><input type="text" placeholder="Your name" autocomplete="name">
          <label>Email</label><input type="email" placeholder="you@example.com" autocomplete="email">
          <label>Password</label><input type="password" placeholder="Create a password" autocomplete="new-password">
          <button type="button" class="btn btn-gold auth-continue">Create Workspace</button>
        </div>
        <button type="button" class="btn btn-outline auth-demo">Continue without account for demo</button>
      </div>`;
    document.querySelector('.hdr')?.insertAdjacentElement('afterend', auth);
    auth.querySelectorAll('.auth-tab').forEach(btn => btn.addEventListener('click', () => {
      const name = btn.dataset.authTab;
      auth.querySelectorAll('.auth-tab').forEach(b => b.classList.toggle('active', b === btn));
      auth.querySelectorAll('.auth-panel').forEach(p => p.classList.toggle('active', p.id === 'auth-' + name));
    }));
    auth.querySelectorAll('.auth-continue,.auth-demo').forEach(btn => btn.addEventListener('click', () => showSitePage('app')));
  }

  function wireTopNav(){
    const brand = document.querySelector('.top-nav .brand');
    if (brand) brand.addEventListener('click', e => { e.preventDefault(); showSitePage('landing'); });
    const links = [...document.querySelectorAll('.top-nav-links a')];
    links.forEach(a => {
      const text = a.textContent.trim().toLowerCase();
      a.addEventListener('click', e => {
        e.preventDefault();
        if (text.includes('features')) showSitePage('landing');
        else if (text.includes('workflow') || text.includes('start')) showSitePage('app');
        else if (text.includes('methodology')) { showSitePage('app'); showPanel('refs', document.querySelector(`[onclick*='refs']`)); }
      });
    });
    const navLinks = document.querySelector('.top-nav-links');
    if (navLinks && !navLinks.querySelector('[data-nav-auth]')) {
      navLinks.insertAdjacentHTML('beforeend', '<button type="button" class="nav-ghost" data-nav-auth="login">Log in</button><button type="button" class="nav-cta" data-nav-auth="signup">Sign up</button>');
      navLinks.querySelectorAll('[data-nav-auth]').forEach(btn => btn.addEventListener('click', () => showSitePage('auth')));
    }
  }

  function wireHeroActions(){
    document.querySelectorAll('.hero-cta .btn, .demo-banner .btn, .nav-cta').forEach(btn => {
      const txt = btn.textContent.trim().toLowerCase();
      if (txt.includes('load demo')) {
        btn.addEventListener('click', () => setTimeout(() => showSitePage('app'), 0));
      } else if (txt.includes('start blank') || txt === 'start') {
        btn.addEventListener('click', e => { e.preventDefault(); showSitePage('app'); showPanel('subject', document.querySelector(`[onclick*='subject']`)); });
      } else if (txt.includes('methodology')) {
        btn.addEventListener('click', e => { e.preventDefault(); showSitePage('app'); showPanel('refs', document.querySelector(`[onclick*='refs']`)); });
      }
    });
  }

  window.showSitePage = function(page, scroll = true){
    ensureAuthPage();
    const isApp = page === 'app';
    const isAuth = page === 'auth';
    document.body.classList.toggle('site-app', isApp);
    document.body.classList.toggle('site-auth', isAuth);
    document.body.classList.toggle('site-landing', !isApp && !isAuth);
    setDisplay(document.querySelector('.hdr'), !isAuth && !isApp);
    setDisplay($('auth-page'), isAuth);
    setDisplay(document.querySelector('main.wrap'), isApp || (!isAuth && !isApp));
    setDisplay(document.querySelector('.app-shell'), isApp);
    document.querySelectorAll('.commercial-strip,.trust-ribbon,.demo-banner,.sell-grid').forEach(el => setDisplay(el, !isApp && !isAuth));
    if (isApp) {
      const active = document.querySelector('.panel.active') || $('subject');
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    if (location.hash && document.querySelector(location.hash + '.panel')) {
      showSitePage('app', false);
      showPanel(location.hash.slice(1), document.querySelector(`[onclick*='${location.hash.slice(1)}']`));
    } else {
      showSitePage('landing', false);
    }
  });
})();
