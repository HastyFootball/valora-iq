...
async function geocodeAll() {
  if (!importedSales.length) { alert('No data imported.'); return; }
  ...
  for (let i = 0; i < importedSales.length; i++) {
    ...
    try {
      const r = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(addr)}&limit=1`, {
        headers: { 'User-Agent': 'AppraiserSuiteDemo/1.0' }
      });
      const d = await r.json();
      if (d && d[0]) { s.lat = parseFloat(d[0].lat); s.lon = parseFloat(d[0].lon); }
    } catch (e) {}
    bar.textContent = `Geocoding… ${i + 1} / ${importedSales.length} (rate-limited per Nominatim policy)`;
    await new Promise(res => setTimeout(res, 1100));
  }
  bar.classList.remove('show');
  alert('Geocoding complete. Run Comp Ranking to see map.');
}
