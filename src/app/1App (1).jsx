...
function parseBathCount(recordOrValue) {
  if (recordOrValue && typeof recordOrValue === 'object') {
    ...
    const total = parseBathCount(recordOrValue.baths_n ?? recordOrValue.baths ?? recordOrValue.bath ?? recordOrValue.total_baths ?? recordOrValue.totalBaths ?? recordOrValue.bathrooms);
...
function rowsFromMapping(headers, rows, map) { return rows.map((cells, i) => { const get = f => map[f] !== undefined && map[f] !== '' ? cells[Number(map[f])] || '' : ''; const fullBaths = toNum(get('full_baths')); const halfBaths = toNum(get('half_baths')); const totalBaths = parseBathCount(get('baths')); const bathTotal = isFinite(totalBaths) ? totalBaths : ((isFinite(fullBaths) ? fullBaths : 0) + (isFinite(halfBaths) ? halfBaths * 0.5 : 0)); return { _id: i, address: get('address'), city: get('city'), state: get('state'), zip: get('zip'), status: get('status') || 'Sold', sale_price_n: toNum(get('sale_price')), gla_n: toNum(get('gla')), site_sf_n: parseSiteArea(get('site_sf'), headers[Number(map.site_sf)] || ''), year_built_n: toNum(get('year_built')), beds_n: toNum(get('beds')), full_baths_n: isFinite(fullBaths) ? fullBaths : null, half_baths_n: isFinite(halfBaths) ? halfBaths : null, baths_n: isFinite(bathTotal) && bathTotal > 0 ? bathTotal : null, sale_date: get('sale_date'), contract_date: get('contract_date'), quality: get('quality'), condition: get('condition'), garage: get('garage'), basement: get('basement'), pool: get('pool'), dom: toNum(get('dom')), list_price_n: toNum(get('list_price')), concessions_n: toNum(get('concessions')), lat: toNum(get('lat')) || null, lon: toNum(get('lon')) || null }; }).filter(r => r.address || r.sale_price_n || r.gla_n); }
