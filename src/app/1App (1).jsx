...
function hasUsableBathValue(v) {
  if (v === null || v === undefined) return false;
  const raw = String(v).trim().toLowerCase();
  if (!raw || raw === '-' || raw === '—' || raw === 'n/a' || raw === 'na' || raw === 'none' || raw === 'null') return false;
  const parsed = parseBathCount(raw);
  return isFinite(parsed) && parsed > 0;
}
function parseBathCount(recordOrValue) {
  if (recordOrValue && typeof recordOrValue === 'object') {
    // Prefer an explicit Total Baths field when it is actually populated.
    // If Total Baths is blank, N/A, dash, zero, or unmapped, fall back to Full Baths + Half Baths.
    const totalRaw = recordOrValue.baths_n ?? recordOrValue.baths ?? recordOrValue.bath ?? recordOrValue.total_baths ?? recordOrValue.totalBaths ?? recordOrValue.bathrooms;
    const total = parseBathCount(totalRaw);
    if (isFinite(total) && total > 0) return total;
    const full = toNum(recordOrValue.full_baths_n ?? recordOrValue.full_baths ?? recordOrValue.fullBaths ?? recordOrValue.full_bath ?? recordOrValue.fullBathrooms);
    const half = toNum(recordOrValue.half_baths_n ?? recordOrValue.half_baths ?? recordOrValue.halfBaths ?? recordOrValue.half_bath ?? recordOrValue.partial_baths ?? recordOrValue.partialBaths);
    if (isFinite(full) || isFinite(half)) return (isFinite(full) ? full : 0) + (isFinite(half) ? half * 0.5 : 0);
    return NaN;
  }
  const raw = String(recordOrValue ?? '').trim().toLowerCase();
  if (!raw) return NaN;

  // MLS exports often store baths as "2 Full, 1 Half", "2F 1H", "2/1", or "2.1".
  // Treat 2.1 as 2 full + 1 half when it looks like a bath-code rather than a decimal total.
  const slash = raw.match(/^(\d+)\s*\/\s*(\d+)$/);
  if (slash) return Number(slash[1]) + Number(slash[2]) * 0.5;
  const decimalCode = raw.match(/^(\d+)\.(\d)$/);
  if (decimalCode && Number(decimalCode[2]) > 0) return Number(decimalCode[1]) + Number(decimalCode[2]) * 0.5;

  const fullMatch = raw.match(/(\d+(?:\.\d+)?)\s*(?:full|fulls|fb|f)/);
  const halfMatch = raw.match(/(\d+(?:\.\d+)?)\s*(?:half|halfs|hb|h|partial)/);
  if (fullMatch || halfMatch) return (fullMatch ? Number(fullMatch[1]) : 0) + (halfMatch ? Number(halfMatch[1]) * 0.5 : 0);
...
async function copyChartImageFromButton(button, label = 'Market chart') {
  const wrap = button?.closest?.('.premium-market-chart-wrap, .chart-card, .panel-card');
  const svg = wrap?.querySelector?.('svg');
  if (!svg) { alert('No chart image found to copy.'); return; }
  try {
    ...
    const width = vb?.width || svg.clientWidth || 1000;
    const height = vb?.height || svg.clientHeight || 450;
    ...
    if (navigator.clipboard && window.ClipboardItem) {
      ...
      setTimeout(() => { button.textContent = 'Copy Graph'; }, 1400);
    } else {
      const a = document.createElement('a');
      a.href = URL.createObjectURL(blob);
      a.download = `${String(label).replace(/[^a-z0-9]+/gi, '_')}.png`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
    }
  } catch (err) {
    alert('Copy failed. Try using the browser print/export option.');
  }
}
...
function isIncompleteSiteInput(value) {
  const raw = String(value ?? '').trim().toLowerCase();
  if (!raw) return true;
  if (raw === '.' || raw === '0' || raw === '0.' || /^\d+\.$/.test(raw)) return true;
  const n = toNum(raw);
  return !isFinite(n) || n <= 0;
}
function siteUnitFromInput(value, unitHint = '') {
  const raw = String(value ?? '').toLowerCase();
  const hint = String(unitHint || '').toLowerCase();
  if (/\b(sf|sq\.?\s*ft|sqft|square\s*feet|square\s*foot)\b/.test(raw) || /\b(sf|sq\.?\s*ft|sqft|square\s*feet|square\s*foot)\b/.test(hint)) return 'sf';
  if (/\b(ac|acre|acres|acreage)\b/.test(raw) || /\b(ac|acre|acres|acreage)\b/.test(hint)) return 'ac';
  const n = toNum(raw);
  return isFinite(n) && n > 0 && n < 100 ? 'ac' : 'sf';
}
function subjectSiteInputValue(subject) {
  if (subject?.siteRaw !== undefined && subject?.siteRaw !== null && subject.siteRaw !== '') return String(subject.siteRaw);
  if (subject?.site) return siteDisplay(subject.site);
  return '';
}
...
function featureValue(record, key) {
  ...
  if (key === 'bath') return parseBathCount(record);
  if (key === 'bed') return Number(record.beds_n ?? record.beds ?? record.bedrooms ?? record.bed ?? record.br);
...
function pairedFeatureRates(sales, featureKey) {
  ...
  for (let i = 0; i < filtered.length; i++) for (let j = i + 1; j < filtered.length; j++) {
    ...
    const isBedBath = featureKey === 'bed' || featureKey === 'bath';
    const glaLimit = isBedBath ? 0.28 : 0.18;
    const ageLimit = isBedBath ? 18 : 12;
    const glaClose = a.gla_n && b.gla_n ? Math.abs(a.gla_n - b.gla_n) / Math.max(a.gla_n, b.gla_n) <= glaLimit : true;
    const ageClose = a.year_built_n && b.year_built_n ? Math.abs(a.year_built_n - b.year_built_n) <= ageLimit : true;
    const qcClose = isBedBath ? true : (Math.abs((ratingNum(a.quality) || 0) - (ratingNum(b.quality) || 0)) <= 1 && Math.abs((ratingNum(a.condition) || 0) - (ratingNum(b.condition) || 0)) <= 1);
...
function matrixSolveGaussian(A, b) {
  const n = A.length;
...
function multivariableFeatureRate(sales, targetKey) {
  const pool = (sales || []).filter(s => s && s.sale_price_n > 0);
  const candidates = ['gla', 'site', 'garage', 'bath', 'bed', 'age'].filter(k => k !== targetKey);
  const targetVals = pool.map(s => featureValue(s, targetKey)).filter(v => isFinite(v));
  const targetUnique = new Set(targetVals.map(v => String(Math.round(v * 1000) / 1000)));
  if (pool.length < 10 || targetVals.length < 8 || targetUnique.size < 2) {
    return { n: targetVals.length, rate: NaN, r2: 0, controls: [], reason: 'insufficient target variation' };
  }
  const usableControls = candidates.filter(k => {
    const vals = pool.map(s => featureValue(s, k)).filter(v => isFinite(v));
    const unique = new Set(vals.map(v => String(Math.round(v * 1000) / 1000)));
    return vals.length >= Math.max(8, pool.length * 0.45) && unique.size >= 2;
  }).slice(0, 4);
  const keys = [targetKey, ...usableControls];
  let rows = pool.map(s => ({ y: s.sale_price_n, xs: keys.map(k => featureValue(s, k)) }))
    .filter(r => isFinite(r.y) && r.xs.every(v => isFinite(v)));
  const minRows = Math.max(12, keys.length + 6);
  if (rows.length < minRows) return { n: rows.length, rate: NaN, r2: 0, controls: usableControls, reason: 'insufficient complete rows' };
  // Filter obvious price outliers for stability.
  const yVals = rows.map(r => r.y);
  const q1 = quantileVal(yVals, 0.25), q3 = quantileVal(yVals, 0.75), iqr = q3 - q1;
  if (isFinite(iqr) && iqr > 0 && rows.length >= 20) rows = rows.filter(r => r.y >= q1 - 1.5 * iqr && r.y <= q3 + 1.5 * iqr);
  if (rows.length < minRows) return { n: rows.length, rate: NaN, r2: 0, controls: usableControls, reason: 'insufficient rows after outlier filter' };
  const means = keys.map((_, j) => meanVal(rows.map(r => r.xs[j])));
  const sds = keys.map((_, j) => stdevVal(rows.map(r => r.xs[j])) || 1);
  if (!isFinite(sds[0]) || sds[0] === 0) return { n: rows.length, rate: NaN, r2: 0, controls: usableControls, reason: 'no target spread' };
  const yMean = meanVal(rows.map(r => r.y));
  const ySd = stdevVal(rows.map(r => r.y)) || 1;
  const X = rows.map(r => [1, ...r.xs.map((v, j) => (v - means[j]) / sds[j])]);
  const y = rows.map(r => (r.y - yMean) / ySd);
  const p = X[0].length;
  ...
  // Tiny ridge term to avoid singular matrices from correlated MLS fields.
  for (let d = 1; d < p; d++) XtX[d][d] += 0.03;
  const beta = matrixSolveGaussian(XtX, Xty);
  if (!beta) return { n: rows.length, rate: NaN, r2: 0, controls: usableControls, reason: 'singular model' };
  ...
  const rate = beta[1] * ySd / sds[0];
  return { n: rows.length, rate, r2, controls: usableControls, method: `Price ~ ${keys.join(' + ')}` };
}
function buildAdjustmentSupportCards({ subject, sales, glaNarData }) {
  // IMPORTANT: adjustment support studies must use the full imported MLS/market pool.
  // Do not filter this study to selected comparable sales; selected comps are only for
  // applying/reconciling the supported rates in the final adjustment grid.
  const studySales = Array.isArray(sales) ? sales.filter(s => s && s.sale_price_n > 0) : [];
  const fallbackSales = studySales;
  const specs = [
    { key: 'gla', title: 'GLA', unit: ' / SqFt', fallback: Number(glaNarData?.rate) || 0, round: 1, maxSignal: 300, format: v => `$${fmt(v, 2)} / SqFt` },
    { key: 'site', title: 'Lot Size', unit: ' / SqFt', fallback: 0, round: 0.05, maxSignal: 20, format: v => `$${fmt(v, 2)} / SqFt` },
    { key: 'garage', title: 'Garage Spaces', unit: ' / Space', fallback: 0, round: 1000, maxSignal: 40000, format: v => money(v) },
    { key: 'bath', title: 'Bath Adjustment', unit: ' / Bath', fallback: 0, round: 1000, maxSignal: 35000, format: v => money(v) },
    { key: 'bed', title: 'Bedroom Adjustment', unit: ' / Bedroom', fallback: 0, round: 1000, maxSignal: 30000, format: v => money(v) }
  ];
  return specs.map(spec => {
    if (spec.key === 'gla' && Number(glaNarData?.rate) > 0) {
      const rate = Number(glaNarData.rate);
      const low = Number(glaNarData.low) > 0 ? Number(glaNarData.low) : rate;
      const high = Number(glaNarData.high) > 0 ? Number(glaNarData.high) : rate;
      const methods = Array.isArray(glaNarData.signals) && glaNarData.signals.length
        ? glaNarData.signals.map(sig => `${sig.method}${sig.detail ? ` (${sig.detail})` : ''}`)
        : [glaNarData.method ? `GLA Study (${glaNarData.method})` : 'Current GLA Study / carried grid rate'];
      const confidenceScore = Number(glaNarData.confidenceScore) || (methods.length >= 3 ? 76 : 58);
      const confidence = confidenceScore >= 75 ? 'Strong' : confidenceScore >= 50 ? 'Moderate' : 'Limited / Single-Method Support';
      const displayRange = Math.abs(high - low) > 0.01 ? `${spec.format(Math.abs(low))} to ${spec.format(Math.abs(high))}` : `${spec.format(Math.abs(rate))} point indication`;
      return {
        ...spec,
        conclusion: rate,
        low,
        ...
        studyCount: fallbackSales.length,
        hasSupportedValue: true,
        hasDirectionalSupport: false,
        unsupported: false,
        displayValue: spec.format(rate),
        displayRange,
        methods,
        supportFailureReasons: [],
        narrative: `GLA support mirrors the current GLA Study conclusion of ${spec.format(rate)}. The displayed evidence card is intentionally tied to the GLA Study so the GLA page, Adjustment Grid, workfile export, and evidence card remain consistent.`
      };
    }
    const paired = pairedFeatureRates(fallbackSales, spec.key), grouped = groupedFeatureRate(fallbackSales, spec.key), robust = robustRegressionBundle(fallbackSales, spec.key), hedonic = multivariableFeatureRate(fallbackSales, spec.key), reg = robust.linear, theil = robust.theil, lad = robust.lad, quantReg = robust.quant, sens = sensitivityFeatureRate(fallbackSales, spec.key);
    ...
    if (hedonic.n >= 12 && isFinite(hedonic.rate) && (hedonic.r2 || 0) >= 0.08) methodSignals.push({ name: `Multivariable regression (${hedonic.n} sales, controls: ${hedonic.controls.join(', ') || 'none'}, R² ${fmt(hedonic.r2, 2)})`, value: hedonic.rate, weight: (hedonic.r2 || 0) >= 0.25 ? 1.35 : 1.05 });
    ...
    const plausibleSignals = methodSignals.filter(m => isFinite(m.value) && Math.abs(m.value) > 0 && (!spec.maxSignal || Math.abs(m.value) <= spec.maxSignal));
    const excludedSignals = methodSignals.filter(m => isFinite(m.value) && spec.maxSignal && Math.abs(m.value) > spec.maxSignal);
    const raw = plausibleSignals.map(m => m.value).filter(v => isFinite(v) && Math.abs(v) > 0);
    const absVals = raw.map(v => Math.abs(v));
    const supportCount = plausibleSignals.length;
    ...
    const r2Bonus = Math.min(22, (Math.max(reg.r2 || 0, theil.r2 || 0, hedonic.r2 || 0)) * 40);
    const confidenceScore = absVals.length ? clampNum((supportCount * 16) + r2Bonus + consistencyBonus + (absVals.length >= 4 ? 10 : 0), 5, 96) : 0;
    const hasSupportedValue = !!(conclusion && supportCount >= 1 && absVals.length >= 1 && confidenceScore >= 20);
    ...
    const confidence = hasSupportedValue
      ? (supportCount === 1 ? 'Limited / Single-Method Support' : confidenceScore >= 75 ? 'Strong' : 'Moderate')
    ...
    const methods = plausibleSignals.map(m => m.name);
    const supportFailureReasons = [];
    if (!raw.length) supportFailureReasons.push(excludedSignals.length ? 'Only extreme/non-typical indications were isolated and were excluded from the dollar conclusion.' : 'No usable market-derived indication was isolated.');
    if (!raw.length && ['bed','bath','site','garage'].includes(spec.key)) {
      const vals = fallbackSales.map(s => featureValue(s, spec.key)).filter(v => isFinite(v));
      const uniqueCount = new Set(vals.map(v => String(Math.round(v * 1000) / 1000))).size;
      if (!vals.length) supportFailureReasons.push(`${spec.title} data was not mapped or parsed from the MLS import.`);
      else if (uniqueCount < 2) supportFailureReasons.push(`${spec.title} has no usable variation in the imported data.`);
      else if (hedonic?.reason) supportFailureReasons.push(`Multivariable model could not be used: ${hedonic.reason}.`);
    }
    ...
    const displayRange = hasSupportedValue && low !== null && high !== null ? (Math.abs(high - low) < 1 ? `${spec.format(conclusion)} point indication` : `${spec.format(low)} to ${spec.format(high)}`) : 'N/A';
    const narrative = hasSupportedValue
      ? `${spec.title} support indicates ${supportCount === 1 ? 'a limited single-method adjustment indication' : 'a reconciled adjustment'} of ${spec.format(conclusion)}. The indicated support range is approximately ${displayRange}. This conclusion is based on ${methods.join(', ').toLowerCase()} and should be reconciled with appraiser judgment, property-specific comparability, and verification from source data.`
      : hasDirectionalSupport
    ...
    return { ...spec, studyCount: fallbackSales.length, conclusion, low, high, confidence, confidenceScore: Math.round(confidenceScore), hasSupportedValue, hasDirectionalSupport, unsupported, displayValue, displayRange, methods, supportFailureReasons, paired, grouped, hedonic, reg, theil, lad, quantReg, sens, narrative };
  });
}
...
// ── CSV import ───────────────────────────────────────────────────────────────
const FIELD_ALIASES = { address: ['address', 'property address', 'street address', 'full address', 'street'], city: ['city', 'municipality', 'city state zip'], state: ['state'], zip: ['zip', 'zipcode', 'zip code', 'postal code'], status: ['status', 'mls status'], sale_price: ['sale price', 'close price', 'sold price', 'sold price $', 'sale amount', 'price', 'sp'], sale_date: ['sale date', 'close date', 'sold date', 'closing date'], contract_date: ['contract date', 'under contract date', 'pending date', 'contract signed date', 'ratified date'], gla: ['gla', 'living area', 'sqft', 'sf', 'sq ft', 'above grade'], site_sf: ['acres', 'acreage', 'lot acres', 'site acres', 'land acres', 'land area', 'lot size', 'site size', 'site area', 'lot sf', 'lot sqft'], year_built: ['year built', 'yr built', 'built', 'year constructed'], beds: ['beds', 'bedrooms', 'br', 'bedroom count', 'num bedrooms', '# bedrooms'], full_baths: ['full baths', 'full bath', 'full bathrooms', 'fbaths', 'fb'], half_baths: ['half baths', 'half bath', 'half bathrooms', 'hbaths', 'hb', 'partial baths', 'partial bath'], baths: ['total baths', 'total bathrooms', 'baths total', 'bathrooms total', 'baths', 'bathrooms', 'ba', 'bath', '# baths'], quality: ['quality', 'q rating', 'q'], condition: ['condition', 'c rating', 'c'], garage: ['garage', 'parking', 'garage spaces', 'garage bays', 'garage count'], basement: ['basement', 'bsmt'], pool: ['pool'], dom: ['dom', 'days on market'], lat: ['lat', 'latitude'], lon: ['lon', 'lng', 'longitude'], list_price: ['list price', 'original list price', 'original price', 'lp', 'olp', 'asking price'], concessions: ['seller concessions', 'seller concession', 'concessions', 'seller paid costs', 'seller credit', 'concession amount'] };
const FIELD_LABELS = { address: 'Address', city: 'City', state: 'State', zip: 'ZIP', status: 'Status', sale_price: 'Sale Price', sale_date: 'Sale Date', contract_date: 'Contract Date', gla: 'GLA', site_sf: 'Site / Lot Size', year_built: 'Year Built', beds: 'Bedrooms', full_baths: 'Full Baths', half_baths: 'Half Baths', baths: 'Total Baths', quality: 'Quality', condition: 'Condition', garage: 'Garage', basement: 'Basement', pool: 'Pool', dom: 'DOM', lat: 'Latitude', lon: 'Longitude', list_price: 'List Price', concessions: 'Concessions $' };
...
function normalizedHeader(h) {
  return String(h || '').toLowerCase().replace(/#/g, ' number ').replace(/&/g, ' and ').replace(/[^a-z0-9]+/g, ' ').trim();
}
function scoreHeaderForField(field, header) {
  const h = normalizedHeader(header);
  if (!h) return 0;
  const exact = s => h === s ? 100 : 0;
  const has = s => h.includes(s);
  const any = arr => arr.some(s => has(s));
  if (field === 'address') {
    if (h === 'address') return 100;
    if (h === 'property address' || h === 'street address' || h === 'full address') return 95;
    if (h.includes('address 2') || h.includes('address line 2')) return -50;
    return has('address') ? 55 : 0;
  }
  if (field === 'city') return exact('city') || (h === 'municipality' ? 90 : 0);
  if (field === 'state') return exact('state');
  if (field === 'zip') return ['zip','zipcode','zip code','postal code'].includes(h) ? 100 : 0;
  if (field === 'status') return h === 'status' || h === 'mls status' ? 100 : 0;

  if (field === 'sale_price') {
    if (any(['sold price per sqft','price per sqft','sold price per square foot'])) return -80;
    if (h === 'sold price' || h === 'close price' || h === 'closing price' || h === 'sale price' || h === 'sold amount') return 120;
    if (has('sold price') || has('close price') || has('sale price')) return 105;
    if (h === 'price' || h === 'sp') return 70;
    if (has('list price') || has('asking price')) return -40;
    return 0;
  }
  if (field === 'list_price') {
    if (h === 'list price' || h === 'original list price' || h === 'asking price') return 110;
    if (has('list price') && !has('per sqft')) return 95;
    return 0;
  }

  if (field === 'sale_date') {
    if (h === 'closing date' || h === 'close date' || h === 'sold date' || h === 'sale date') return 110;
    return (has('closing date') || has('close date') || has('sold date') || has('sale date')) ? 90 : 0;
  }
  if (field === 'contract_date') return (h === 'contract date' || h === 'pending date' || h === 'under contract date' || h === 'ratified date') ? 110 : 0;

  if (field === 'gla') {
    if (h === 'total finished sqft' || h === 'total finished sq ft' || h === 'total finished square feet') return 125;
    if (h === 'gla' || h === 'gross living area' || h === 'living area' || h === 'heated sqft' || h === 'heated sq ft') return 120;
    if ((has('finished') || has('living') || has('heated')) && (has('sqft') || has('sq ft') || has('square feet'))) return 105;
    if (h === 'sq ft' || h === 'sqft') return 65;
    if (h === 'area') return -60;
    return 0;
  }

  if (field === 'site_sf') {
    if (h === 'approx number of acres' || h === 'approx acres' || h === 'approx acre' || h === 'acres' || h === 'acreage') return 125;
    if (has('acre')) return 115;
    if ((has('lot') || has('site') || has('land')) && (has('size') || has('area') || has('sf') || has('sqft') || has('sq ft'))) return 105;
    if (h === 'area') return -80;
    return 0;
  }

  if (field === 'year_built') {
    if (h === 'approx year built' || h === 'year built' || h === 'yr built') return 115;
    if (has('year built') || has('built')) return 85;
    return 0;
  }
  if (field === 'beds') return ['number br','br','number bedrooms','bedrooms','beds','bedroom count'].includes(h) ? 120 : (has('bedroom') && !has('bath') ? 90 : 0);
  if (field === 'full_baths') return ['number fb','fb','full bath','full baths','full bathrooms','number full baths'].includes(h) ? 125 : ((has('full') && has('bath')) ? 105 : 0);
  if (field === 'half_baths') return ['number of half baths','number half baths','half bath','half baths','hb','partial baths'].includes(h) ? 125 : (((has('half') || has('partial')) && has('bath')) ? 105 : 0);
  if (field === 'baths') {
    if (has('half') || has('full') || h === 'number fb' || h === 'number of half baths') return -60;
    if (['total baths','total bathrooms','bathrooms total','baths total','baths','bathrooms','ba'].includes(h)) return 115;
    return 0;
  }
  if (field === 'garage') {
    if (h === 'garage total capacity' || h === 'garage spaces' || h === 'garage count' || h === 'garage bays') return 120;
    return has('garage') ? 95 : 0;
  }
  if (field === 'dom') return h === 'cumulative dom' || h === 'dom' || h === 'days on market' ? 115 : 0;
  if (field === 'quality') return h === 'quality' || h === 'q rating' || h === 'q' ? 100 : 0;
  if (field === 'condition') return h === 'condition' || h === 'c rating' || h === 'c' ? 100 : 0;
  if (field === 'basement') return has('basement') || has('bsmt') ? 100 : 0;
  if (field === 'pool') return h === 'pool' ? 100 : 0;
  if (field === 'lat') return h === 'lat' || h === 'latitude' ? 100 : 0;
  if (field === 'lon') return h === 'lon' || h === 'lng' || h === 'longitude' ? 100 : 0;
  if (field === 'concessions') return any(['seller concessions','seller concession','seller contributions','seller contribution','seller paid costs','seller credit','concession amount']) ? 110 : 0;
  return 0;
}
function autoMapHeaders(headers) {
  ...
  Object.keys(FIELD_LABELS).forEach(field => {
    ...
    headers.forEach((h, i) => {
      const score = scoreHeaderForField(field, h);
      if (score > best.score) best = { score, idx: String(i) };
    });
    if (best.score >= 50) map[field] = best.idx;
  });
  return map;
}
function rowsFromMapping(headers, rows, map) { return rows.map((cells, i) => { const get = f => map[f] !== undefined && map[f] !== '' ? cells[Number(map[f])] || '' : ''; const fullBaths = toNum(get('full_baths')); const halfBaths = toNum(get('half_baths')); const totalBathRaw = get('baths'); const totalBaths = parseBathCount(totalBathRaw); const hasTotalBaths = isFinite(totalBaths) && totalBaths > 0; const bathTotal = hasTotalBaths ? totalBaths : ((isFinite(fullBaths) ? fullBaths : 0) + (isFinite(halfBaths) ? halfBaths * 0.5 : 0)); return { _id: i, address: get('address'), city: get('city'), state: get('state'), zip: get('zip'), status: get('status') || 'Sold', sale_price_n: toNum(get('sale_price')), gla_n: toNum(get('gla')), site_sf_n: parseSiteArea(get('site_sf'), headers[Number(map.site_sf)] || ''), year_built_n: toNum(get('year_built')), beds_n: toNum(get('beds')), full_baths_n: isFinite(fullBaths) ? fullBaths : null, half_baths_n: isFinite(halfBaths) ? halfBaths : null, baths_n: isFinite(bathTotal) && bathTotal > 0 ? bathTotal : null, bath_import_source: hasTotalBaths ? 'total baths field' : ((isFinite(fullBaths) || isFinite(halfBaths)) ? 'full + half baths' : 'missing'), sale_date: get('sale_date'), contract_date: get('contract_date'), quality: get('quality'), condition: get('condition'), garage: get('garage'), basement: get('basement'), pool: get('pool'), dom: toNum(get('dom')), list_price_n: toNum(get('list_price')), concessions_n: toNum(get('concessions')), lat: toNum(get('lat')) || null, lon: toNum(get('lon')) || null }; }).filter(r => r.address || r.sale_price_n || r.gla_n); }
...
function DashboardShell({ persona, session }) {
  ...
  function openProject(project) {
    ...
    setAdjRows(Array.isArray(project.data?.adjRows) ? project.data.adjRows : []);
    setMarketStudyState(project.data?.marketStudyState || { mode: 'rolling3', minSales: 1, ran: false, showRaw: true, showModified: true });
    setGlaStudyState(project.data?.glaStudyState || { mtRate: project.data?.mtNarData?.monthly || 0, regResult: null, pairedRows: [{ pa: '', ga: '', pb: '', gb: '' }], pairedResult: null, applyInputs: { sg: project.data?.subject?.gla || '', cg: '', rate: project.data?.glaNarData?.rate || '', cp: '' }, applyResult: null });
    setRoute(persona === 'appraiser' ? 'Subject Property' : 'Property Overview');
  }
...
function Workspace({ persona, tab, setRoute, subject, setSubject, sales, setSales, selectedComps, setSelectedComps, adjRows, setAdjRows, glaNarData, setGlaNarData, mtNarData, setMtNarData, marketStudyState, setMarketStudyState, glaStudyState, setGlaStudyState, siteValueState, setSiteValueState, adjustmentDefaults, setAdjustmentDefaults, projects, projectsLoading, currentProjectId, currentProjectName, newProject, loadDemoProject, openProject, deleteProject, saveProject, fetchProjects }) {
  ...
  if (tab === 'Adjustment Grid') return <Adjustments selectedComps={selectedComps} sales={sales} subject={subject} adjRows={adjRows} setAdjRows={setAdjRows} adjustmentDefaults={adjustmentDefaults} setAdjustmentDefaults={setAdjustmentDefaults} glaNarData={glaNarData} />;
...
function SubjectForm({ persona, subject, setSubject, setRoute }) {
  ...
  const [message, setMessage] = useState('');
  const [siteDraft, setSiteDraft] = useState(() => subjectSiteInputValue(subject));
  useEffect(() => {
    setSiteDraft(subjectSiteInputValue(subject));
  }, [subject?.address, subject?.city, subject?.site, subject?.siteRaw]);
  const fields = [
  ...
  function update(key, type, value) {
    setSaved(false);
    if (key === 'site') {
      const raw = String(value ?? '');
      setSiteDraft(raw);
      const sf = parseSiteArea(raw, '');
      const unit = siteUnitFromInput(raw, '');
      setSubject({
        ...subject,
        siteRaw: raw,
        site: isIncompleteSiteInput(raw) || !isFinite(sf) ? '' : sf,
        siteUnit: unit
      });
      return;
    }
    setSubject({ ...subject, [key]: type === 'number' ? (value === '' ? '' : toNum(value)) : value });
  }
      ...
      <section className="panel-card">
        ...
        <div className="form-grid">
          {fields.map(([key, label, type = 'text']) => (
            <label key={key}>{label}<input type={type === 'site' ? 'text' : type} value={key === 'site' ? siteDraft : (subject[key] ?? '')} placeholder={key === 'site' ? 'Example: 0.25 acres or 10,890 sf' : ''} onChange={e => update(key, type, e.target.value)} onBlur={e => { if (key === 'site' && subject.site) setSiteDraft(String(e.target.value || '').trim()); }} />{key === 'site' && <small>Acres are assumed for MLS-style values under 100 unless SF is typed. Current calculation value: {subject.site && Number(subject.site) > 0 ? siteDisplay(subject.site) : 'not set yet'}.</small>}</label>
          ))}
        </div>
        <div className="btn-row">
            ...
            <g key={p.key}>
              {showRaw && (
                <g className="hover-point">
                  <circle cx={p.x} cy={p.rawY} r="4.5" fill="#d6b04a" stroke="#071933" strokeWidth="2"><title>{p.key} Raw Median: {money(p.y)} ({p.n} sales)</title></circle>
                  <g className="svg-hover-label" transform={`translate(${p.x}, ${Math.max(42, p.rawY - 34)})`}>
                    <rect x="-86" y="-25" width="172" height="34" rx="10" />
                    <text x="0" y="-11" textAnchor="middle">{p.key}</text>
                    <text x="0" y="3" textAnchor="middle">Raw: {money(Math.round(p.y))} · {p.n} sales</text>
                  </g>
                </g>
              )}
              {showModified && (
                <g className="hover-point">
                  <circle cx={p.x} cy={p.modY} r="5" fill="#4de1ff" stroke="#071933" strokeWidth="2"><title>{p.key} Modified Median: {money(p.yMod)} ({p.n} sales)</title></circle>
                  <g className="svg-hover-label" transform={`translate(${p.x}, ${Math.max(42, p.modY - 34)})`}>
                    <rect x="-98" y="-25" width="196" height="34" rx="10" />
                    <text x="0" y="-11" textAnchor="middle">{p.key}</text>
                    <text x="0" y="3" textAnchor="middle">Modified: {money(Math.round(p.yMod))} · {p.n} sales</text>
                  </g>
                </g>
              )}
...
function SimpleTrendChart({ title, subtitle, points = [], valueFormatter = v => fmt(v), bar = false, lineLabel = 'Trend', empty = 'Insufficient data.' }) {
  ...
  return (
    <div className="premium-market-chart-wrap mini-exhibit-chart">
      <div className="premium-chart-toolbar"><div><p className="eyebrow">{subtitle}</p><h3>{title}</h3></div><div className="chart-legend"><button className="btn ghost small copy-chart-btn" onClick={e => copyChartImageFromButton(e.currentTarget, title)}>Copy Graph</button><span><i className="modified" /> {lineLabel}</span></div></div>
      <svg className="premium-market-svg mini-market-svg" viewBox={`0 0 ${w} ${h}`} role="img" aria-label={title}>
        ...
        {!bar && coords.map((p, i) => (
          <g className="hover-point" key={p.key}>
            <circle cx={p.cx} cy={p.cy} r="5" fill="#d6b04a" stroke="#071933" strokeWidth="2"><title>{p.key}: {valueFormatter(p.y)} ({p.n || 0} records)</title></circle>
            <g className="svg-hover-label" transform={`translate(${p.cx}, ${Math.max(42, p.cy - 32)})`}>
              <rect x="-82" y="-25" width="164" height="34" rx="10" />
              <text x="0" y="-11" textAnchor="middle">{p.key}</text>
              <text x="0" y="3" textAnchor="middle">{valueFormatter(p.y)} · {p.n || 0} records</text>
            </g>
          </g>
        ))}
    ...
    <div className="dash-page batch2-market-page">
      ...
      {ran ? (
        <>
          ...
          <section className="market-map-grid market-pressure-only">
...
function GLAStudy({ sales, subject, setGlaNarData, glaStudyState, setGlaStudyState, setAdjustmentDefaults, adjustmentDefaults }) {
  ...
  function runRegression() {
    const adjustedSales = sales
      .filter(sale => !isNaN(sale.gla_n) && !isNaN(sale.sale_price_n) && sale.gla_n > 0 && sale.sale_price_n > 0)
      .map(sale => {
        let adjPrice = sale.sale_price_n;
        if (mtRate !== 0 && subject.effdate && sale.sale_date) {
          const m = monthsBetween(subject.effdate, sale.sale_date);
          adjPrice *= (1 + mtRate / 100 * m);
        }
        return { ...sale, sale_price_n: adjPrice };
      });
    if (adjustedSales.length < 3) { alert('Need at least 3 sales with GLA and price data.'); return; }
    const paired = pairedFeatureRates(adjustedSales, 'gla');
    const grouped = groupedFeatureRate(adjustedSales, 'gla');
    const robust = robustRegressionBundle(adjustedSales, 'gla');
    const sens = sensitivityFeatureRate(adjustedSales, 'gla');
    const signals = [];
    if (paired.n >= 2 && isFinite(paired.median) && Math.abs(paired.median) <= 300) signals.push({ method: 'Paired Sales', rate: paired.median, n: paired.n, detail: `${paired.n} pairings` });
    if (grouped.groups?.length >= 2 && isFinite(grouped.rate) && Math.abs(grouped.rate) <= 300) signals.push({ method: 'Grouped Data', rate: grouped.rate, n: grouped.groups.length, detail: `${grouped.groups.length} groups` });
    if (robust.linear.n >= 4 && isFinite(robust.linear.rate) && Math.abs(robust.linear.rate) <= 300) signals.push({ method: 'Linear Regression', rate: robust.linear.rate, n: robust.linear.n, detail: `R² ${fmt(robust.linear.r2, 3)}` });
    if (robust.theil.n >= 5 && isFinite(robust.theil.rate) && Math.abs(robust.theil.rate) <= 300) signals.push({ method: 'Theil-Sen Regression', rate: robust.theil.rate, n: robust.theil.n, detail: `R² ${fmt(robust.theil.r2, 3)}` });
    if (robust.lad.n >= 6 && isFinite(robust.lad.rate) && Math.abs(robust.lad.rate) <= 300) signals.push({ method: 'LAD Regression', rate: robust.lad.rate, n: robust.lad.n, detail: 'least absolute deviation' });
    if (robust.quant.n >= 6 && isFinite(robust.quant.rate) && Math.abs(robust.quant.rate) <= 300) signals.push({ method: 'Quantile Regression', rate: robust.quant.rate, n: robust.quant.n, detail: 'median quantile' });
    if (sens.n >= 4 && isFinite(sens.rate) && Math.abs(sens.rate) <= 300) signals.push({ method: 'Sensitivity Analysis', rate: sens.rate, n: sens.n, detail: `${sens.n} iterations` });
    if (!signals.length) { alert('The GLA study did not isolate a usable supported $/SF indication. Try reviewing data quality, outliers, or paired sales.'); return; }
    const rateVals = signals.map(x => Math.abs(x.rate));
    const reconciled = median(rateVals);
    const low = quantileVal(rateVals, 0.20);
    const high = quantileVal(rateVals, 0.80);
    const bestR2 = Math.max(robust.linear.r2 || 0, robust.theil.r2 || 0);
    const confidenceScore = clampNum((signals.length * 14) + Math.min(24, bestR2 * 40) + (rateVals.length >= 4 ? 12 : 0), 10, 96);
    const rel = confidenceScore >= 75 ? 'Strong multi-method support' : confidenceScore >= 50 ? 'Moderate support — reconcile with paired sales' : 'Limited support — use caution';
    const next = { slope: reconciled, r2: bestR2, n: adjustedSales.length, rel, low, high, confidenceScore: Math.round(confidenceScore), signals };
    updateState({ regResult: next, applyInputs: { ...applyInputs, rate: reconciled } });
    setGlaNarData({ rate: reconciled, method: `robust multi-method GLA study (${signals.map(x => x.method).join(', ')})`, low, high, confidenceScore: Math.round(confidenceScore), signals });
    setAdjustmentDefaults(prev => ({ ...(prev || {}), glaRate: Number(reconciled || 0) }));
  }
  ...
  function calcPaired() {
    ...
    setGlaNarData({ rate: med, method: 'paired sales analysis', low: Math.min(...rates), high: Math.max(...rates), confidenceScore: rates.length >= 3 ? 58 : 45, signals: [{ method: 'Manual Paired Sales', detail: `${rates.length} pair(s)`, rate: med }] });
  ...
  return (
    <div className="dash-page">
      ...
      <section className="panel-card">
        ...
        {regResult && <><div className="metric-grid four" style={{ marginTop: 16 }}><div><b>${fmt(regResult.slope, 2)}/SF</b><span>Reconciled Rate</span></div><div><b>{regResult.low !== undefined ? `$${fmt(regResult.low, 2)} – $${fmt(regResult.high, 2)}` : fmt(regResult.r2, 3)}</b><span>{regResult.low !== undefined ? 'Support Range' : 'R²'}</span></div><div><b>{regResult.n}</b><span>Sales Used</span></div><div><b style={{ fontSize: '1rem', lineHeight: 1.3 }}>{regResult.rel}</b><span>Reliability</span></div></div>{regResult.signals?.length ? <div className="table-card embedded" style={{ marginTop: 16 }}><table><thead><tr><th>Method</th><th>Indication</th><th>Support</th></tr></thead><tbody>{regResult.signals.map((sig, i) => <tr key={i}><td>{sig.method}</td><td>${fmt(Math.abs(sig.rate), 2)}/SF</td><td>{sig.detail}</td></tr>)}</tbody></table></div> : null}</>}
      </section>
      <section className="panel-card">
...
function AdjustmentEvidenceCard({ card, onUse }) {
  ...
  return (
    <article className={`adjustment-evidence-card ${statusClass}`}>
      ...
      <small className="study-pool-note">Study pool: {fmt(card.studyCount || 0)} imported sale records. Selected comps are not used to calculate this support card.</small>
...
function Adjustments({ selectedComps, sales, subject, adjRows, setAdjRows, adjustmentDefaults, setAdjustmentDefaults, glaNarData }) {
  ...
  // Condition and quality adjustments are intentionally not auto-generated.
  // These are too property-specific to support reliably from broad regression/grouping.
  const condRate = 0;
  const qualRate = 0;
  ...
  const mirroredGlaNarData = glaNarData?.rate ? glaNarData : { rate: glaRate };
  const supportCards = useMemo(() => buildAdjustmentSupportCards({ subject, sales, glaNarData: mirroredGlaNarData }), [subject, sales, glaRate, glaNarData]);
  ...
  function build() {
    ...
    const rows = useSales.map((s, i) => {
      ...
      const saleBeds = featureValue(s, 'bed');
      const saleBaths = featureValue(s, 'bath');
      const bedroomAdj = bedroomRate && subject.beds && isFinite(saleBeds) ? (Number(subject.beds) - saleBeds) * bedroomRate : 0;
      const subjectBaths = parseBathCount(subject);
      const bathAdj = bathRate && isFinite(subjectBaths) && isFinite(saleBaths) ? (subjectBaths - saleBaths) * bathRate : 0;
      const otherAdj = garageAdj + bedroomAdj + bathAdj;
      const totalAdj = timeAdj + glaAdj + siteAdj + ageAdj + otherAdj;
      const noteParts = [];
      ...
      if (supportByKey.bath?.hasSupportedValue && bathAdj) noteParts.push(`Bath supported at ${supportByKey.bath.format(supportByKey.bath.conclusion)}`);
      if (supportByKey.bed?.hasSupportedValue && bedroomAdj) noteParts.push(`Bedroom supported at ${supportByKey.bed.format(supportByKey.bed.conclusion)}`);
  ...
  function editAdj(i, k, v) {
    const next = adjRows.map((r, j) => {
      ...
      updated.totalAdj = updated.timeAdj + updated.glaAdj + updated.siteAdj + updated.ageAdj + updated.otherAdj;
  ...
  return (
    <div className="dash-page batch3-adjustment-page">
      ...
      <section className="panel-card">
        <div className="card-head"><div><p className="eyebrow">Methodology Comparison</p><h2>Adjustment evidence cards</h2><p className="muted">These studies use the full imported MLS dataset. Selected comps only receive the final applied adjustments in the grid.</p></div><span className="status-pill">Full-pool studies</span></div>
      ...
      {adjRows.length > 0 && (
        <>
          ...
          <section className="table-card" style={{ overflowX: 'auto' }}>
            <div className="card-head"><h2>Adjustment Grid</h2><button className="btn ghost small" onClick={() => { const csv = ['Rank,Address,Sale Price,Date,Score,Time,GLA,Site,Age,Other,Net Adj,Adjusted,Notes'].concat(adjRows.map(r => [r.rank, `"${r.address}"`, r.price, r.date, Math.round(r.score), Math.round(r.timeAdj), Math.round(r.glaAdj), Math.round(r.siteAdj), Math.round(r.ageAdj), Math.round(r.otherAdj), Math.round(r.totalAdj), Math.round(r.adjusted), `"${r.note}"`].join(','))).join('\n'); const b = new Blob([csv], { type: 'text/csv' }); const a = document.createElement('a'); a.href = URL.createObjectURL(b); a.download = 'adjustment_grid.csv'; a.click(); }}>Export CSV ⇩</button></div>
            <table><thead><tr><th>#</th><th>Comp</th><th>Sale Price</th><th>Date</th><th>Score</th><th>Time</th><th>GLA</th><th>Site</th><th>Age</th><th>Other</th><th>Net Adj</th><th>Adjusted</th><th>Notes</th></tr></thead><tbody>{adjRows.map((r, i) => <tr key={i}><td>{r.rank}</td><td>{r.address || '—'}</td><td>{money(r.price)}</td><td>{r.date || '—'}</td><td>{fmt(r.score, 0)}</td>{['timeAdj', 'glaAdj', 'siteAdj', 'ageAdj', 'otherAdj'].map(k => <td key={k}><input className="cell-input" defaultValue={Math.round(r[k])} onBlur={e => editAdj(i, k, e.target.value)} style={{ width: 80 }} /></td>)}<td style={{ color: r.totalAdj >= 0 ? 'var(--green)' : 'var(--red)' }}>{fmtD(r.totalAdj)}</td><td><b>{money(r.adjusted)}</b></td><td><input className="cell-input wide" defaultValue={r.note} onBlur={e => { const next = adjRows.map((x, j) => j === i ? { ...x, note: e.target.value } : x); setAdjRows(next); }} style={{ width: 190 }} /></td></tr>)}</tbody></table>
          </section>
        </>
      )}
    </div>
...
function ExportWorkfile({ subject, sales, adjRows, glaNarData, mtNarData, saveProject }) {
  ...
  const supportCards = useMemo(() => buildAdjustmentSupportCards({ subject, sales, glaNarData }), [subject, sales, glaNarData]);
