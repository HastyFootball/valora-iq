...
function buildAdjustmentSupportCards({ subject, sales, adjRows, glaNarData }) {
  // Use the full imported market pool for support studies. Selected comps can be too narrow
  // and may hide bath/bed/garage signals even when the full MLS export has enough variation.
...
function Adjustments({ selectedComps, sales, subject, adjRows, setAdjRows, adjustmentDefaults, setAdjustmentDefaults, glaNarData }) {
  ...
  const supportCards = useMemo(() => buildAdjustmentSupportCards({ subject, sales, adjRows: selectedRows.map((s, i) => ({ rank: i + 1, address: s.address })), glaNarData: mirroredGlaNarData }), [subject, sales, selectedRows, glaRate, glaNarData]);
