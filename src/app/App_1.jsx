...
function featureValue(record, key) {
  ...
  if (key === 'site') return Number(record.site_sf_n ?? record.site);
...
function buildAdjustmentSupportCards({ subject, sales, adjRows, glaNarData }) {
  ...
  const specs = [
    ...
    { key: 'garage', title: 'Garage Spaces', unit: ' / Space', fallback: 10000, round: 1000, format: v => money(v) },
    { key: 'bath', title: 'Bath Adjustment', unit: ' / Bath', fallback: 10000, round: 1000, format: v => money(v) },
    { key: 'bed', title: 'Bedroom Adjustment', unit: ' / Bedroom', fallback: 5000, round: 1000, format: v => money(v) },
    { key: 'condition', title: 'Condition Rating', unit: ' / Rating Step', fallback: 5000, round: 1000, format: v => money(v) },
    { key: 'quality', title: 'Quality Rating', unit: ' / Rating Step', fallback: 5000, round: 1000, format: v => money(v) }
  ];
  return specs.map(spec => {
    const paired = pairedFeatureRates(fallbackSales, spec.key), grouped = groupedFeatureRate(fallbackSales, spec.key), reg = regressionFeatureRate(fallbackSales, spec.key), sens = sensitivityFeatureRate(fallbackSales, spec.key);
    const raw = [paired.median, grouped.rate, reg.rate, sens.rate].filter(v => isFinite(v) && Math.abs(v) > 0);
    const absVals = raw.map(v => Math.abs(v));
    const conclusionRaw = absVals.length ? median(absVals) : Math.abs(spec.fallback || 0);
    const conclusion = spec.round < 1 ? Math.round(conclusionRaw / spec.round) * spec.round : roundTo(conclusionRaw, spec.round);
    const lowRaw = absVals.length ? quantileVal(absVals, 0.20) : conclusion * 0.75, highRaw = absVals.length ? quantileVal(absVals, 0.80) : conclusion * 1.25;
    const supportCount = [paired.n >= 2, grouped.groups?.length >= 2, reg.n >= 4, sens.n >= 4].filter(Boolean).length;
    const spread = conclusion ? Math.abs(highRaw - lowRaw) / Math.max(1, conclusion) : 9;
    const confidenceScore = clampNum((supportCount * 22) + Math.min(24, (reg.r2 || 0) * 40) + (spread < 0.6 ? 18 : 6), 12, 96);
    const methods = [];
    if (paired.n >= 2) methods.push(`Paired sales (${paired.n} pairings)`);
    if (grouped.groups?.length >= 2) methods.push(`Grouped data (${grouped.groups.length} groups)`);
    if (reg.n >= 4) methods.push(`Linear regression (${reg.n} sales, R² ${fmt(reg.r2, 2)})`);
    if (sens.n >= 4) methods.push(`Sensitivity analysis (${sens.n} iterations)`);
    if (!methods.length) methods.push('Fallback professional review required');
    const confidence = confidenceScore >= 75 ? 'Strong' : confidenceScore >= 50 ? 'Moderate' : confidenceScore >= 30 ? 'Limited' : 'Directional';
    const low = spec.round < 1 ? Math.round(lowRaw / spec.round) * spec.round : roundTo(lowRaw, spec.round);
    const high = spec.round < 1 ? Math.round(highRaw / spec.round) * spec.round : roundTo(highRaw, spec.round);
    return { ...spec, conclusion, low, high, confidence, confidenceScore: Math.round(confidenceScore), methods, paired, grouped, reg, sens, narrative: `${spec.title} support indicates a reconciled adjustment of ${spec.format(conclusion)}. The indicated support range is approximately ${spec.format(Math.abs(low))} to ${spec.format(Math.abs(high))}. This conclusion is based on ${methods.join(', ').toLowerCase()} and should be reconciled with appraiser judgment, property-specific comparability, and verification from source data.` };
  });
}
...
function supportCardHtml(card) {
  const methodHtml = card.methods.map(m => `<li>${escHtml(m)}</li>`).join('');
  const ticks = [0, 25, 50, 75, 100].map(x => `<i style="left:${x}%"></i>`).join('');
  return `<article class="support-card"><header><span>${escHtml(card.title)}</span><strong>${card.format(card.conclusion)}</strong></header><div class="range-row"><b>Low: ${card.format(card.low)}</b><b>High: ${card.format(card.high)}</b></div><div class="dot-range">${ticks}<em style="left:${clampNum(card.confidenceScore, 2, 98)}%"></em></div><div class="confidence"><b>${escHtml(card.confidence)}</b><span>${card.confidenceScore}/100 support confidence</span></div><h4>Calculated Support Methods</h4><ul>${methodHtml}</ul><p>${escHtml(card.narrative)}</p></article>`;
}
function buildWorkfileHtml({ subject, sales, adjRows, sections, supportCards, marketSupport }) {
  ...
  const dataRows = (sales || []).slice(0, 80).map(s => `<tr><td>${escHtml(s.address || '')}</td><td>${escHtml(s.status || '')}</td><td>${money(s.sale_price_n)}</td><td>${escHtml(s.sale_date || '')}</td><td>${fmt(s.gla_n)}</td><td>${fmt(s.site_sf_n)}</td><td>${escHtml(s.garage || '')}</td><td>${fmt(s.dom)}</td><td>${escHtml(s.quality || '')}/${escHtml(s.condition || '')}</td></tr>`).join('');
...
function rowsFromMapping(headers, rows, map) { return rows.map((cells, i) => { const get = f => map[f] !== undefined && map[f] !== '' ? cells[Number(map[f])] || '' : ''; return { _id: i, address: get('address'), city: get('city'), state: get('state'), zip: get('zip'), status: get('status') || 'Sold', sale_price_n: toNum(get('sale_price')), gla_n: toNum(get('gla')), site_sf_n: toNum(get('site_sf')), year_built_n: toNum(get('year_built')), sale_date: get('sale_date'), contract_date: get('contract_date'), quality: get('quality'), condition: get('condition'), garage: get('garage'), basement: get('basement'), pool: get('pool'), dom: toNum(get('dom')), concessions_n: toNum(get('concessions')), lat: toNum(get('lat')) || null, lon: toNum(get('lon')) || null }; }).filter(r => r.address || r.sale_price_n || r.gla_n); }
...
function demoWorkspace() {
  ...
  const subject = {
    ...
    gla: 2250, site: 10890, year: 2018, beds: 4, baths: 2.5,
        ...
        <section className="table-card">
          ...
          <table>
            ...
            <tbody>{preview.slice(0, 25).map((r, i) => (
              <tr key={i}><td>{r.address || '—'}</td><td>{r.city || '—'}</td><td>{r.status || '—'}</td><td>{money(r.sale_price_n)}</td><td>{r.sale_date || '—'}</td><td>{r.contract_date || '—'}</td><td>{r.gla_n || '—'}</td><td>{r.site_sf_n || '—'}</td><td>{r.year_built_n || '—'}</td><td>{r.quality || '—'}</td><td>{r.condition || '—'}</td><td>{r.lat && r.lon ? '✓' : r.geocode_status || '—'}</td></tr>
            ))}</tbody>
          </table>
        </section>
      )}
...
function analyzeMarketIntelligence(sales, series, mode) {
  const valid = (sales || []).filter(s => s.sale_price_n > 0 && (s.contract_date || s.sale_date));
  const sold = valid.filter(s => String(s.status || '').toLowerCase().includes('sold') || String(s.status || '').toLowerCase().includes('sld') || !s.status);
  const active = (sales || []).filter(s => /active|act|listed/i.test(String(s.status || '')));
  const pending = (sales || []).filter(s => /pending|under contract|contingent|uc/i.test(String(s.status || '')));
  ...
  const domTrend = (() => {
    const sorted = valid.filter(s => s.dom >= 0).sort((a, b) => dateToMonths(a.contract_date || a.sale_date) - dateToMonths(b.contract_date || b.sale_date));
  ...
  const monthlySales = series?.points?.length ? meanVal(series.points.map(p => p.n)) : 0;
  ...
  const confidence = clampNum((series?.points?.length || 0) * 10 + (valid.length >= 12 ? 20 : valid.length) + (domVals.length ? 10 : 0) - priceVolatility * 30, 18, 96);
  const velocityScore = clampNum(62 + monthly * 6 + (pending.length * 4) - Math.max(0, (medDom || 45) - 45) * 0.42 - Math.max(0, monthsSupply - 4) * 7 - priceVolatility * 45, 4, 98);
...
function MarketConditions({ persona, sales, setMtNarData, marketStudyState, setMarketStudyState, setAdjustmentDefaults }) {
  ...
  function generate() {
    ...
    setMtNarData({ monthly: series.monthly, dir, narrative: intel.narrative, velocityScore: intel.velocityScore, monthsSupply: intel.monthsSupply });
...
function AdjustmentEvidenceCard({ card, onUse }) {
  return (
    <article className={`adjustment-evidence-card ${card.confidenceScore >= 70 ? 'strong' : card.confidenceScore >= 45 ? 'moderate' : 'limited'}`}>
      <div className="evidence-head">
        <div><span>{card.title}</span><strong>{card.format(card.conclusion)}</strong></div>
        <b>{card.confidenceScore}/100</b>
      </div>
      <div className="confidence-bar"><i style={{ width: `${card.confidenceScore}%` }} /></div>
      <p>Support range {card.format(card.low)} to {card.format(card.high)}. {card.confidence} confidence based on available data.</p>
      <ul>{card.methods.map(m => <li key={m}>{m}</li>)}</ul>
      <div className="btn-row"><button className="btn ghost small" onClick={() => onUse(card)}>Use this indication</button><button className="btn ghost small" onClick={() => navigator.clipboard?.writeText(card.narrative)}>Copy defense</button></div>
    </article>
  );
}
...
function Adjustments({ selectedComps, sales, subject, adjRows, setAdjRows, adjustmentDefaults, setAdjustmentDefaults }) {
  ...
  function build() {
    ...
    const rows = useSales.map((s, i) => {
      ...
      const noteParts = [];
      if (supportByKey.gla?.confidenceScore >= 50 && glaAdj) noteParts.push(`GLA supported at ${supportByKey.gla.format(supportByKey.gla.conclusion)}`);
      if (supportByKey.garage?.confidenceScore >= 50 && garageAdj) noteParts.push(`Garage supported at ${supportByKey.garage.format(supportByKey.garage.conclusion)}`);
      if (supportByKey.condition?.confidenceScore >= 50 && condAdj) noteParts.push(`Condition supported at ${supportByKey.condition.format(supportByKey.condition.conclusion)}`);
  ...
  return (
    <div className="dash-page batch3-adjustment-page">
      <section className="premium-welcome batch3-hero">
        ...
        <div className="comp-hero-score"><span>Supported Items</span><b>{supportCards.filter(c => c.confidenceScore >= 50).length}</b><em>/{supportCards.length}</em></div>
      </section>
...
function ExportWorkfile({ subject, sales, adjRows, glaNarData, mtNarData, saveProject }) {
  ...
  const completeness = Math.round(clampNum((subject?.address ? 15 : 0) + ((sales || []).length ? 25 : 0) + (marketSupport.valid.length >= 3 ? 15 : 0) + ((glaNarData?.rate || supportCards.find(c => c.key === 'gla')?.conclusion) ? 15 : 0) + ((adjRows || []).length ? 20 : 0) + (supportCards.filter(c => c.confidenceScore >= 50).length >= 3 ? 10 : 0), 0, 100));
  ...
  return (
    <div className="dash-page premium-workfile-page">
      ...
      <section className="panel-card">
        <div className="card-head"><div><p className="eyebrow">Adjustment Support Studio</p><h2>Preview support cards</h2></div><span className="status-pill">{supportCards.filter(c => c.confidenceScore >= 50).length}/{supportCards.length} supported</span></div>
        <div className="workfile-support-preview">
          {supportCards.map(card => (<article className="workfile-support-card" key={card.key}><div><span>{card.title}</span><strong>{card.format(card.conclusion)}</strong></div><div className="confidence-bar"><b style={{ width: `${card.confidenceScore}%` }} /></div><p>{card.confidence} support · Range {card.format(card.low)} to {card.format(card.high)}</p><small>{card.methods.join(' · ')}</small></article>))}
        </div>
      </section>
    </div>
  );
