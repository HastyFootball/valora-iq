...
function ProjectIntelligencePanel({ persona, sales = [], selectedComps = [], adjRows = [], setRoute }) {
  ...
  const health = !hasAnyProjectData ? 0 : Math.min(96,
    (sales.length ? 25 : 0) +
    (sales.length && missingGeo === 0 ? 15 : 0) +
    (sales.length && missingQC === 0 ? 15 : 0) +
    (selected ? 25 : 0) +
    (adjRows.length ? 16 : 0)
  );
  ...
  return (
    <section className="intelligence-grid">
      <article className="intelligence-card primary">
        ...
        <p className="muted">{health === 0 ? 'No project data yet. Confidence will build as you import MLS data, geocode, review Q/C, select comps, and build support.' : 'Confidence improves as MLS data, geocoding, Q/C, comp selection, and adjustment support are completed.'}</p>
      </article>
