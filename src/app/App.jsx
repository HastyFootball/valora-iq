    ...
    <div className="dash-page">
      ...
      {ran ? (
        <section className="chart-card">
          ...
          <section className="panel-card mc-ai-panel" style={{ marginTop: 16 }}>
            <p className="eyebrow">Market Insight</p>
            <h2>How to read this</h2>
            <p className="muted">
              <strong>MoM %</strong> shows month-to-month market movement. <strong>Change to Latest</strong> estimates the cumulative adjustment from that sale period to the latest period in the dataset. Contract date is preferred when available because it better reflects when buyer and seller agreed to price. Use professional judgment before applying any adjustment.
            </p>
          </section>
        </section>
      ) : (
...
// ── Comp Ranking ──────────────────────────────────────────────────────────────
function compMatchInsight(s, subject) {
  const tags = [];
  if (subject.gla && s.gla_n) {
    const diff = Math.abs(s.gla_n - subject.gla) / subject.gla;
    tags.push(diff <= 0.1 ? 'Strong GLA match' : diff <= 0.2 ? 'Usable GLA bracket' : 'GLA variance');
  }
  if (subject.year && s.year_built_n) {
    const diff = Math.abs(s.year_built_n - subject.year);
    tags.push(diff <= 5 ? 'Similar age' : diff <= 15 ? 'Age adjustment likely' : 'Age outlier');
  }
  if (subject.qual && s.quality) tags.push(ratingNum(subject.qual) === ratingNum(s.quality) ? 'Same quality' : 'Quality differs');
  if (subject.cond && s.condition) tags.push(ratingNum(subject.cond) === ratingNum(s.condition) ? 'Same condition' : 'Condition differs');
  return tags.slice(0, 4);
}

function matchBarScore(label, raw) {
  const v = Math.max(0, Math.min(100, Math.round(raw)));
  return <div className="match-bar" key={label}><span>{label}</span><i><b style={{ width: `${v}%` }} /></i><em>{v}</em></div>;
}
          ...
          <article className={`rank-card ${selectedSet.has(s._key) ? 'selected' : ''}`} key={s._key}>
            ...
            <div>
              ...
              <div className="rank-meta">
                ...
                <span>Sold Date <b>{s.sale_date || '—'}</b></span>
                <span>Contract Date <b>{s.contract_date || '—'}</b></span>
              ...
              <div className="comp-insight-row">
                {compMatchInsight(s, subject).map(tag => <em key={tag}>{tag}</em>)}
              </div>
              <div className="match-bars">
                {matchBarScore('GLA', subject.gla && s.gla_n ? 100 - Math.min(100, Math.abs(s.gla_n - subject.gla) / subject.gla * 250) : 35)}
                {matchBarScore('Distance', s._distance !== null ? 100 - Math.min(100, s._distance / 5 * 100) : 35)}
                {matchBarScore('Date', s.sale_date ? 100 - Math.min(100, monthsBetween(subject.effdate || new Date().toISOString().slice(0, 10), s.sale_date) / 24 * 100) : 40)}
              </div>
