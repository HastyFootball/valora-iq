...
function featureValue(record, key) {
  ...
  if (key === 'bath') return Number(record.baths_n ?? record.baths ?? record.bath ?? record.full_baths ?? record.fullBaths ?? record.total_baths);
...
function rowsFromMapping(headers, rows, map) { return rows.map((cells, i) => { const get = f => map[f] !== undefined && map[f] !== '' ? cells[Number(map[f])] || '' : ''; return { _id: i, address: get('address'), city: get('city'), state: get('state'), zip: get('zip'), status: get('status') || 'Sold', sale_price_n: toNum(get('sale_price')), gla_n: toNum(get('gla')), site_sf_n: parseSiteArea(get('site_sf'), headers[Number(map.site_sf)] || ''), year_built_n: toNum(get('year_built')), beds_n: toNum(get('beds')), baths_n: toNum(get('baths')), sale_date: get('sale_date'), contract_date: get('contract_date'), quality: get('quality'), condition: get('condition'), garage: get('garage'), basement: get('basement'), pool: get('pool'), dom: toNum(get('dom')), list_price_n: toNum(get('list_price')), concessions_n: toNum(get('concessions')), lat: toNum(get('lat')) || null, lon: toNum(get('lon')) || null }; }).filter(r => r.address || r.sale_price_n || r.gla_n); }
