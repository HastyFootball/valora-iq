...
function buildAdjustmentSupportCards({ subject, sales, glaNarData }) {
  ...
  return specs.map(spec => {
    ...
    return { ...spec, studyCount: fallbackSales.length, conclusion, low, high, confidence, confidenceScore: Math.round(confidenceScore), hasSupportedValue, hasDirectionalSupport, unsupported, displayValue, displayRange, methods, supportFailureReasons, paired, grouped, reg, theil, lad, quantReg, sens, narrative };
  });
}
...
function SubjectForm({ persona, subject, setSubject, setRoute }) {
  ...
  function update(key, type, value) {
    ...
    if (key === 'site') {
      ...
      setSiteDraft(raw);
      const sf = parseSiteArea(raw, subject.siteUnit || '');
      const unit = siteUnitFromInput(raw, subject.siteUnit || '');
      setSubject({
        ...subject,
        siteRaw: raw,
