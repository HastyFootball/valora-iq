...
// ── Dashboard shell ───────────────────────────────────────────────────────────
function DashboardShell({ persona }) {
    ...
    <div className={`dashboard-shell ${collapsed ? 'is-collapsed' : ''}`}>
      ...
      <main className="dashboard-main">
        <header className="topbar">
          ...
          <div className="topbar-actions"><button className="btn ghost">Help</button><button className="avatar">VQ</button></div>
        </header>
...
// ── App root ──────────────────────────────────────────────────────────────────
function App() {
  useEffect(() => { const s = document.createElement('style'); s.textContent = chartStyle; document.head.appendChild(s); }, []);
  const path = usePath();
  if (path === '/login') return <Auth type="login" />;
  if (path === '/signup') return <Auth type="signup" />;
  if (path.startsWith('/appraiser')) return <DashboardShell persona="appraiser" />;
  if (path.startsWith('/agent')) return <DashboardShell persona="agent" />;
  return <Landing />;
}
