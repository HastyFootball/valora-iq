...
function parseBathCount(recordOrValue) {
  if (recordOrValue && typeof recordOrValue === 'object') {
    const total = toNum(recordOrValue.baths_n ?? recordOrValue.baths ?? recordOrValue.bath ?? recordOrValue.total_baths ?? recordOrValue.totalBaths);
    if (isFinite(total)) return total;
    const full = toNum(recordOrValue.full_baths_n ?? recordOrValue.full_baths ?? recordOrValue.fullBaths ?? recordOrValue.full_bath);
    const half = toNum(recordOrValue.half_baths_n ?? recordOrValue.half_baths ?? recordOrValue.halfBaths ?? recordOrValue.half_bath);
  ...
  const fullMatch = raw.match(/(\d+(?:\.\d+)?)\s*(?:full|f)/);
  const halfMatch = raw.match(/(\d+(?:\.\d+)?)\s*(?:half|h)/);
...
function GLAStudy({ sales, subject, setGlaNarData, glaStudyState, setGlaStudyState, setAdjustmentDefaults, adjustmentDefaults }) {
  ...
  function runRegression() {
    ...
    setGlaNarData({ rate: reconciled, method: `robust multi-method GLA study (${signals.map(x => x.method).join(', ')})` });
...
function Adjustments({ selectedComps, sales, subject, adjRows, setAdjRows, adjustmentDefaults, setAdjustmentDefaults }) {
  ...
  function build() {
    ...
    const rows = useSales.map((s, i) => {
      ...
      const bathAdj = bathRate && subject.baths && isFinite(saleBaths) ? (Number(subject.baths) - saleBaths) * bathRate : 0;
