...
function WorkflowProgress({ persona, sales = [], selectedComps = [], adjRows = [], currentProjectName = '', setRoute }) {
  const isAppraiser = persona === 'appraiser';
  const steps = [
    ...
    { label: isAppraiser ? 'Market Conditions' : 'Market Snapshot', route: isAppraiser ? 'Market Conditions' : 'Market Snapshot', done: false, help: 'Run trend' },
...
function ProjectIntelligencePanel({ persona, sales = [], selectedComps = [], adjRows = [], setRoute }) {
  ...
  const health = Math.max(15, Math.min(96, 35 + (sales.length ? 20 : 0) + (missingGeo === 0 && sales.length ? 15 : 0) + (missingQC === 0 && sales.length ? 15 : 0) + (selected ? 15 : 0) + (adjRows.length ? 10 : 0)));
  ...
  return (
    <section className="intelligence-grid">
      <article className="intelligence-card primary">
        <p className="eyebrow">Analysis Confidence</p>
        <div className="health-ring" style={{ '--score': `${health}%` }}>
          <strong>{health}</strong><span>/100</span>
        </div>
        <p className="muted">Confidence improves as MLS data, geocoding, Q/C, comp selection, and adjustment support are completed.</p>
      </article>
...
function AppraiserHome({ sales, projects = [], selectedComps = [], adjRows = [], currentProjectName = '', setRoute, newProject, openProject, deleteProject }) {
  ...
  return (
    <div className="dash-page">
      ...
      <WorkflowProgress persona="appraiser" sales={sales} selectedComps={selectedComps} adjRows={adjRows} currentProjectName={currentProjectName} setRoute={setRoute} />
      <ProjectIntelligencePanel persona="appraiser" sales={sales} selectedComps={selectedComps} adjRows={adjRows} setRoute={setRoute} />
...
function MarketLineChart({ points, max, showRaw = true, showModified = true }) {
  ...
  const w = 960, h = 390, padL = 74, padR = 36, padT = 38, padB = 58;
  const allVals = points
    .flatMap(p => [showRaw ? p.y : null, showModified ? p.yMod : null])
    .filter(v => isFinite(v));

  const minVal = Math.min(...allVals);
  const maxVal = Math.max(max || 1, ...allVals);
  const padding = Math.max(1000, (maxVal - minVal) * 0.12);
  ...
  const yFor = value => padT + ((maxY - value) / span) * chartH;
  ...
  const activePath = showModified ? modPath : rawPath;
  const areaPath = activePath ? `${activePath} L ${coords[coords.length - 1].x.toFixed(1)} ${(h - padB).toFixed(1)} L ${coords[0].x.toFixed(1)} ${(h - padB).toFixed(1)} Z` : '';

  return (
    <div className="premium-market-chart-wrap">
      ...
      <svg className="premium-market-svg" viewBox={`0 0 ${w} ${h}`} role="img" aria-label="Premium market conditions trend graph">
        <defs>
          <linearGradient id="marketGlowFill" x1="0" y1="0" x2="0" y2="1">
        ...
        <rect x={padL} y={padT} width={chartW} height={chartH} rx="18" className="chart-plot-bg" />
        {yTicks.map((tick, i) => {
          const y = yFor(tick);
          return (
            <g key={i} className="chart-grid-row">
              <line x1={padL} y1={y} x2={w - padR} y2={y} />
              <text x={padL - 14} y={y + 4} textAnchor="end">{money(Math.round(tick))}</text>
            </g>
          );
        })}

        {areaPath && <path d={areaPath} className="chart-area-fill" />}
        {showRaw && <path d={rawPath} className="raw-trend-line" />}
        {showModified && <path d={modPath} className="modified-trend-line" filter="url(#lineGlow)" />}

        {coords.map((p, i) => {
          const activeY = showModified ? p.modY : p.rawY;
          const activeVal = showModified ? p.yMod : p.y;
          const showLabel = i === 0 || i === coords.length - 1 || i % 2 === 0;
          return (
            <g key={p.key} className="chart-point-group">
              {showRaw && <circle cx={p.x} cy={p.rawY} r="4.5" className="raw-dot"><title>{p.key} Raw Median: {money(p.y)} ({p.n} sales)</title></circle>}
              {showModified && <circle cx={p.x} cy={p.modY} r="5.5" className="modified-dot"><title>{p.key} Modified Median: {money(p.yMod)} ({p.n} sales)</title></circle>}
              {showLabel && <text x={p.x} y={Math.max(18, activeY - 14)} textAnchor="middle" className="premium-price-label">{money(Math.round(activeVal))}</text>}
              <text x={p.x} y={h - 22} textAnchor="middle" className="premium-x-label">{p.key}</text>
            </g>
          );
        })}
      </svg>
    ...
    <div className="dash-page">
      <section className="panel-card">
        ...
        <p className="muted max">
          View raw and modified market trends based on contract date when available, with sold date as a fallback. Track month-to-month movement and cumulative change to the latest period for appraisal market adjustment support.
        </p>
        ...
        {ran && (
          <>
            <div className="metric-grid four premium-metrics">
            ...
            <div className="mc-insight-strip">
              <div><span>Trend Confidence</span><b>{series.points.length >= 6 ? 'High' : series.points.length >= 3 ? 'Moderate' : 'Low'}</b></div>
              <div><span>Date Basis</span><b>Contract date preferred</b></div>
              <div><span>Suggested Support</span><b>{Math.abs(series.monthly) < 0.1 ? 'No material adjustment' : `${series.monthly > 0 ? '+' : ''}${series.monthly.toFixed(2)}% / month`}</b></div>
            </div>
          </>
        )}
      </section>
      ...
      {ran ? (
        <section className="chart-card">
          <h2>Median Sale Price Trend by Contract Date</h2>
          ...
          <section className="market-intel-grid" style={{ marginTop: 16 }}>
            <article className="market-intel-card hero">
              <p className="eyebrow">Market Intelligence Brief</p>
              <h2>{series.monthly > 0.1 ? 'Upward market pressure detected' : series.monthly < -0.1 ? 'Downward market pressure detected' : 'Market appears broadly stable'}</h2>
              <p>
                ValoraIQ analyzed {series.points.length} market periods and identified an average monthly movement of <strong>{series.monthly.toFixed(3)}%</strong>. Contract date is preferred when available because it better reflects the date buyers and sellers agreed to price.
              </p>
            </article>
            <article className="market-intel-card">
              <span>Trend Confidence</span>
              <b>{series.points.length >= 8 ? 'High' : series.points.length >= 4 ? 'Moderate' : 'Low'}</b>
              <p>{series.points.length >= 8 ? 'Enough periods exist to support a credible trend read.' : 'Consider additional data or corroborating support.'}</p>
            </article>
            <article className="market-intel-card">
              <span>Adjustment Signal</span>
              <b>{Math.abs(series.monthly) < 0.1 ? 'Minimal' : `${series.monthly > 0 ? '+' : ''}${series.monthly.toFixed(2)}% / mo`}</b>
              <p>Use professional judgment before applying any adjustment.</p>
            </article>
          </section>
        </section>
      ) : (
